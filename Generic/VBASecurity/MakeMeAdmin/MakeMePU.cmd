...
REM  after temporarily adding that user to the local Power Users group.
REM  Any program launched from that command shell will also run with
REM  Power User privileges.
REM
REM  You will be prompted for two passwords in two separate command shells:
REM  first, for the password of the local administrator or PU account, and
...
REM  which the user will be a member of the Power Users group.)
REM
REM  CUSTOMIZATION:
...
REM
REM  * _Admin_ : the name of the account that can make changes
REM              to the Power Users group (usu. "Administrator" unless you renamed the
...
set _Group_="Power Users"
set _Prog_="cmd.exe /k Title *** %* as Power User *** && cd c:\ && color 5F"
